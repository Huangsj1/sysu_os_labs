...
; bts (bytes test set) 将寄存器对应的位设置到CF寄存器中，并置位
; uint32 asm_atomic_bts(uint32 *memory);
; 将CF寄存器的值存储到eax寄存器中
asm_atomic_bts:
    push ebp
    mov ebp, esp
    ...
    xor eax, eax
    mov ebx, [ebp + 4 * 2]  ; memory address
    lock bts dword [ebx], 0 ; memory
    lahf    ; (load ah flags)将标志寄存器的值加载到ah寄存器中
    mov al, ah
    and al, 1
    xor ah, ah
